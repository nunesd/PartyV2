...
public class PartyService {
    ...
    public GenericOutput delete(Long partyId) {
        ...
        foundCandidateOnDelete(partyId);
    ...
    private void foundCandidateOnDelete (Long electionId) {
    	List<CandidateOutput> listCdOut = candidateClientService.getAll();
    	boolean foundItem = false;
    	
    	for(int i =0; i < listCdOut.size() && !foundItem; i++) {
    		Long numberCdOut = listCdOut.get(i).getPartyOutput().getId();
    	...
    	if (foundItem) {
    		throw new GenericOutputException("Cannot delete party with Candidate");
		}
    }
